...
in {
  ...
  applications.${name} = {
    ...
    templates.garageObjectStore."${objectStoreName}" = {
      ...
      # operator-managed `garage-backup` cluster (its woodpecker bucket/key are
      # provisioned and the creds are reflected as woodpecker-s3-secret-key).
      awsDefaultRegion = "backup";
      destinationPath = "s3://woodpecker/backups";
